...
HTML = f"""<!doctype html><html><head><meta charset='utf-8'><meta name='viewport' content='width=device-width,initial-scale=1'><style>html,body,iframe{{width:100%;height:100%;min-height:680px;margin:0;border:0;background:#f4f6f8}}</style></head><body><iframe id='u' title='AVOCarbon SharePoint Upload'></iframe><script>function p(v){{const d=v?.structuredContent||v||{{}};const q=new URLSearchParams(d);document.getElementById('u').src='{BASE_URL}/upload?'+q.toString()}}p(window.openai?.toolOutput);window.addEventListener('openai:set_globals',e=>p(e.detail?.globals?.toolOutput));</script></body></html>"""
